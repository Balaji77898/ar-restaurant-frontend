...
export default function PaymentPage() {
  ...
  const saveOrderHistory = () => {
    ...
    const newOrder = {
      ...
      items: cart.map(item => ({ ...item, status: "Preparing" })),
      subtotal,
      gst,
      total,
    };
 ...
 const downloadInvoice = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FBF6EE] via-[#F5EFE0] to-[#FBF6EE] pb-40 relative">

      {/* HEADER */}
      <div className="sticky top-0 z-50 bg-gradient-to-r from-[#5D1616] via-[#7B1F1F] to-[#5D1616] shadow-[0_4px_20px_rgba(0,0,0,0.3)]">
        <div className="px-4 py-4 flex items-center justify-between text-[#FFF8E1]">
          <button
      ...
      {/* ORDER SUMMARY */}
      {cart.length > 0 && (
        <section className="px-4 mt-6 animate-slide-in-up">
